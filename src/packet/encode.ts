...
export async function encodeHeader(destId: string, maskingIv: Buffer, header: IHeader): Promise<Buffer> {
  const ctx = await aes.create(Uint8Array.from(fromHex(destId).slice(0, MASKING_KEY_SIZE)), Uint8Array.from(maskingIv));
...
export async function decodeHeader(srcId: string, maskingIv: Buffer, data: Buffer): Promise<[IHeader, Buffer]> {
  const ctx = await aes.create(Uint8Array.from(fromHex(srcId).slice(0, MASKING_KEY_SIZE)), Uint8Array.from(maskingIv));
